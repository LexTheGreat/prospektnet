...
Public Class AccountData
    ...
    Public Sub CreateCharacter(ByVal curAccount As AccountBase)
        Try
            ...
            Exit Sub
        Catch ex As Exception
            Console.WriteLine("Error: " & ex.ToString & " (In: Accounts.Data.CreateCharacter)")
            Exit Sub
        End Try
    End Sub
    ...
    Public Sub SaveAccounts()
        Try
            ...
            End If
        Catch ex As Exception
            Console.WriteLine("Error: " & ex.ToString & " (In: Accounts.Data.SaveAccounts")
        End Try
    End Sub

    Public Sub SaveAccount(ByRef saccount As AccountBase)
        Try
            Files.WriteXML(pathAccounts & saccount.Email & ".xml", saccount)
            Account(GetAccountIndex(saccount.Email)).Base = saccount
            Exit Sub
        Catch ex As Exception
            Console.WriteLine("Error: " & ex.ToString & " (In: Accounts.Data.SaveAccount)")
            Exit Sub
        End Try
    End Sub
