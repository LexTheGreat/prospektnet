...
Public Class NPCData
    ...
    Public Sub LoadNPCs()
        Try
            ...
            If Directory.Exists(pathNPCs) Then
                ...
                For Each fileName In fileEntries
                    loadNPC = DirectCast(Files.ReadXML(fileName, loadNPC), NPCBase)
                    NPCCount = i + 1
        ...
        Catch ex As Exception
            Console.WriteLine("Error: " & ex.ToString & " (In: NPCs.Data.LoadNPCs")
        End Try
    End Sub

    Public Sub SaveNPC(ByVal snpc As NPCBase)
        Try
            Files.WriteXML(pathNPCs & snpc.Name & ".xml", snpc)
            NPC(GetNPCIndex(snpc.Name)).Base = snpc
            Exit Sub
        Catch ex As Exception
            Console.WriteLine("Error: " & ex.ToString & " (In: NPCs.Data.SaveNPC)")
            Exit Sub
        End Try
    End Sub
